from rewards.robo_instruct.roboeval.misc.bounded_subprocess import run as bounded_run
from rewards.robo_instruct.roboeval.benchmark.rtl import TraceElement, SPECIAL_DELIM
from rewards.robo_instruct.roboeval.misc.utils import read_benchmark
...
import sys
import time
from rewards.robo_instruct.roboeval.benchmark.simulator import Robot
from rewards.robo_instruct.roboeval.benchmark.tasks.{task_name} import tests
...
def evaluate_program(taskname, program, tests) -> Union[bool, str]:
    ...
    for num_state, test in enumerate(tests):
        ...
        try:
            ...
            is_sat = constraint_cb(trace_elements, False)
            if not is_sat:
                return_val = False
        except Exception as e:
            if "Robot" in str(e):
